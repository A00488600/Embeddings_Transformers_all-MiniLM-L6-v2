...
df = df.set_index("Target").join(
    df1.set_index("Target"), lsuffix="_Source", rsuffix="_Target"
)
df["Target"] = df.index
df = df.reset_index(drop=True)
print(df)
...
for e in edge_data:
    ...
    got_net.add_edge(
        src, dst, value=w
    )  # ,color = "#c79910") # if you  want a solide colour for edges
